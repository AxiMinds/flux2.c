...
static flux_text_encoder_t *text_encoder_load(FILE *f) {
    flux_text_encoder_t *enc = calloc(1, sizeof(flux_text_encoder_t));
    if (!enc) return NULL;

    /* Read config */
    uint32_t config[5];
    if (fread(config, sizeof(uint32_t), 5, f) != 5) {
        free(enc);
        return NULL;
    }

    enc->hidden_size = config[0];
    enc->num_layers = config[1];
    enc->num_heads = config[2];
    enc->vocab_size = config[3];
    enc->max_seq_len = config[4];

    /* Read embeddings */
    enc->token_embeddings = (float *)malloc(enc->vocab_size * enc->hidden_size * sizeof(float));
    if (fread(enc->token_embeddings, sizeof(float),
              enc->vocab_size * enc->hidden_size, f) !=
        (size_t)(enc->vocab_size * enc->hidden_size)) {
        free(enc->token_embeddings);
        free(enc);
        return NULL;
    }

    enc->position_embeddings = (float *)malloc(enc->max_seq_len * enc->hidden_size * sizeof(float));
    if (fread(enc->position_embeddings, sizeof(float),
              enc->max_seq_len * enc->hidden_size, f) !=
        (size_t)(enc->max_seq_len * enc->hidden_size)) {
        free(enc->token_embeddings);
    ...
    /* Output layers */
    enc->output_layers[0] = 10;
    enc->output_layers[1] = 20;
    enc->output_layers[2] = 30;
...
flux_ctx *flux_load(const char *model_path) {
    ...
    /* Check magic */
    char magic[4];
    if (fread(magic, 1, 4, f) != 4 || memcmp(magic, FLUX_MAGIC, 4) != 0) {
        set_error("Invalid model file (bad magic)");
        fclose(f);
        return NULL;
    }
    ...
    /* Check version */
    uint32_t version;
    if (fread(&version, sizeof(uint32_t), 1, f) != 1 || version > FLUX_VERSION) {
        set_error("Unsupported model version");
        fclose(f);
        return NULL;
    }
    ...
    /* Read config */
    uint32_t config[4];
    if (fread(config, sizeof(uint32_t), 4, f) != 4) {
        set_error("Failed to read config");
        goto error;
    }

    ctx->max_width = config[0];
    ctx->max_height = config[1];
    ctx->default_steps = config[2];
    ctx->default_guidance = *(float *)&config[3];

    /* Read model name */
    uint8_t name_len;
    if (fread(&name_len, 1, 1, f) != 1) goto error;
    if (fread(ctx->model_name, 1, name_len, f) != name_len) goto error;
    ctx->model_name[name_len] = '\0';

    /* Read version string */
    uint8_t ver_len;
    if (fread(&ver_len, 1, 1, f) != 1) goto error;
    if (fread(ctx->model_version, 1, ver_len, f) != ver_len) goto error;
    ctx->model_version[ver_len] = '\0';

    /* Read component flags */
    uint32_t flags;
    if (fread(&flags, sizeof(uint32_t), 1, f) != 1) goto error;

    /* Load components based on flags */

    /* Tokenizer (flag bit 0) */
    if (flags & 0x01) {
        /* Tokenizer is stored inline */
        /* For now, skip - would need tokenizer data here */
    }

    /* Text encoder (flag bit 1) */
    if (flags & 0x02) {
        ctx->text_encoder = text_encoder_load(f);
        if (!ctx->text_encoder) {
            set_error("Failed to load text encoder");
            goto error;
        }
    }
    ...
    /* VAE (flag bit 2) */
    if (flags & 0x04) {
        ctx->vae = flux_vae_load(f);
        if (!ctx->vae) {
            set_error("Failed to load VAE");
            goto error;
        }
    }

    /* Transformer (flag bit 3) */
    if (flags & 0x08) {
        ctx->transformer = flux_transformer_load(f);
        if (!ctx->transformer) {
...
/* ========================================================================
 * Load from HuggingFace-style directory with safetensors files
 * ======================================================================== */
